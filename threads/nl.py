import threading, time, os, logging, wave
import pyaudio
import numpy as np
...
class NoiseLib(threading.Thread):
    ...
    def generate_noise(self):
        noise_frames_count = np.floor(self.out_fs * self.noise_length)
    ...
    def generate_chirp(self):
        t = np.linspace(0,
                        self.chirp_length,
                        num=np.floor(self.out_fs * self.chirp_length))
    ...
    def load_wave(self):
        # 获取raw目录下所有音频文件名
        raw_wave_dir = os.path.join(".\waves", "raw")
        raw_wave_filenames = []
        for root, dirs, files in os.walk(raw_wave_dir):  # 字符串前+r表示不含转义字符
            for file in files:
                raw_wave_filenames.append(file)

        # 根据文件名读取音频文件
        self.raw_waves = []
        try:
            for filename in raw_wave_filenames:
                wf = wave.open(os.path.join(raw_wave_dir, filename), "rb")
                print(wf.getparams())
                ...
                audio_clip = signal.resample(
                    np.frombuffer(bytes_buffer, dtype=np.int16),
                    int(settings.OUT_FS / framerate * nframes))  # ???
                audio_clip = audio_clip / (2**15 - 1)

                self.raw_waves.append(
                    [filename, 2, sampwidth, settings.OUT_FS,
                     audio_clip])  # ???
        except:
            raise TypeError("Can't read wave file!")

        noise_clip = self.raw_waves[-1][-1]
        # global_var.noise_pool.clear()
        # global_var.noise_pool.put(noise_clip)

        self.raw_waves[-1][-1] = Modulate.am_modulate(noise_clip)
        # self.raw_waves[-1][-1] = (noise_clip,noise_clip)
